...
事件 ID 4624 ——「成功登录账户」—— 是 Windows 每次完成身份认证时写入 [`Security` 通道](/zh/blog/what-is-an-evtx-file)的记录。在大多数工作站上,它是流量最大的单一记录,也是几乎所有应急响应调查的脊柱。会读它是必备技能。
...
- 同一来源紧接其前的 [**4625**](/zh/blog/detecting-4625-brute-force) —— 一阵失败之后的一次成功,是教科书式的「爆破成功」签名。
- [**4672**](/zh/blog/event-id-4672-special-privileges)(已分配特殊权限)—— 标记授予了 `SeDebugPrivilege` 等权限的登录,适合用来找特权账号的使用情况。
